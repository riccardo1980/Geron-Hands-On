...
class LinearRegression:
    ...
    def fit(self, X, y):
        ...
        with tf.Graph().as_default():
            ...
            tX = tf.placeholder(tf.float32, shape=(
                None, self._n_features), name='X')
            ...
            fval = tf.matmul(tf.math.add(tf.matmul(tf.transpose(
                theta), H), - 2.0 * tf.transpose(b)), theta) + c
            mse = tf.squeeze(tf.divide(fval, tf.cast(
                n_examples, tf.float32)), name='mse')
            ...
            with tf.Session() as sess:
                ...
                for epoch in range(self._n_epochs):
                    for X_batch, y_batch in data.batch(X, y, epoch):
                        _, mse_batch = sess.run([training_op, mse], feed_dict={
                                                tX: X_batch, ty: y_batch})

                    if epoch % self._print_every == 0 or epoch == self._n_epochs-1:
                        print('Epoch {: 5d}, MSE {:4.2e}'.format(
                            epoch, mse_batch))

                        save_path = saver.save(sess, 'tmp/model.ckpt')
                        file_writer.add_summary(mse_summary.eval(
                            feed_dict={tX: X_batch, ty: y_batch}), epoch)
    ...
    def predict(self, X):
        ...
        with graph.as_default():
            tX = tf.placeholder(tf.float32, shape=(
                None, self._n_features), name='X')
            theta = tf.placeholder(tf.float32, shape=(
                self._n_features, 1), name='theta')
            pred = tf.matmul(tX, theta)

            with tf.Session() as sess:
                predictions = sess.run(
                    pred, feed_dict={tX: X, theta: self._theta})
...
class LogisticRegression:
    ...
    def fit(self, X, y):
        ...
        with tf.Graph().as_default():
            ...
            predicted_proba = tf.math.sigmoid(
                tf.matmul(tX, theta), 'predicted_proba')
            ...
            with tf.Session() as sess:
                ...
                for epoch in range(self._n_epochs):
                    for X_batch, y_batch in data.batch(X, y, epoch):
                        _, loss_batch = sess.run([training_op, loss], feed_dict={
                                                 tX: X_batch, ty: y_batch})

                    if epoch % self._print_every == 0 or epoch == self._n_epochs-1:
                        print('Epoch {: 5d}, loss {:4.2e}'.format(
                            epoch, loss_batch))

                        save_path = saver.save(sess, 'tmp/model.ckpt')
                        file_writer.add_summary(loss_summary.eval(
                            feed_dict={tX: X_batch, ty: y_batch}), epoch)
    ...
    def predict(self, X):
        ...
        with graph.as_default():
            ...
            with tf.Session() as sess:
                predictions = sess.run(classes, feed_dict={
                                       tX: X, theta: self._theta})
